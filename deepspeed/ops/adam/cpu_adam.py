...
from deepspeed.ops.op_builder import (
    CPUAdamBuilder,
    SonnetAdamBuilder
)
...
class SonnetCPUAdam(torch.optim.Optimizer):
    optimizer_id = 0
    ...
    def __del__(self):
        # need to destroy the C++ object explicitly to avoid a memory leak when deepspeed.initialize
        # is used multiple times in the same process (notebook or pytest worker)
        self.sn_opt_adam.destroy_adam(self.opt_id)

    def __setstate__(self, state):
        super(SonnetCPUAdam,self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', Flase)
